    ...
    class RobotControl:
        ...
        def ada_dinding_depan(self, threshold=0.5):
        ...
        def ada_dinding_kanan(self, threshold=0.5):
            # Right side laser scan index (close to 0)
            kanan = self.get_laser(0)

            if kanan < threshold:
                rospy.loginfo(f"Dinding di kanan kedetek {kanan}m, stopping...")
                self.stop_robot()
                return True
            rospy.loginfo(f"Gak ad dinding di kanan {threshold}m, {kanan}..")
            return False

        def ada_dinding_kiri(self, threshold=0.5):
            # Left side laser scan index (close to 719)
            kiri = self.get_laser(719)

            if kiri < threshold:
                rospy.loginfo(f"Dinding di kiri kedetek {kiri}m, stopping...")
                self.stop_robot()
                return True
            rospy.loginfo(f"Gak ad dinding di kiri {threshold}m, {kiri}..")
            return False
    ...
    def main():
        ...
        try:
            ...
            while 1:
                if not RC.ada_dinding_depan(TRESHOLD_DINDING):
                    print("1. ==========<LURUS>==============")
                    RC.move_straight()
                if not RC.ada_dinding_kanan(TRESHOLD_DINDING):
                    print("2. ==========<BELOK KANAN>==============")
                    RC.turn("clockwise", 1, BELOK_WAKTU)
                    RC.move_straight()
                if not RC.ada_dinding_kiri(TRESHOLD_DINDING):
                    print("2. ==========<BELOK KIRI>==============")
                    RC.turn("counter-clockwise", 1, BELOK_WAKTU)
                    RC.move_straight()
